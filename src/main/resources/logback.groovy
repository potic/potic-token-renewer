...
appender('FILE', RollingFileAppender) {
    file = "${System.getenv('LOG_PATH') ?: 'logs'}/potic-token-renewer.log"
    ...
    rollingPolicy(SizeAndTimeBasedRollingPolicy) {
        FileNamePattern = "${System.getenv('LOG_PATH') ?: 'logs'}/potic-token-renewer.%d{yyyy-MM-dd}.%i.log"
        MaxHistory = 30
        MaxFileSize = "256MB"
        TotalSizeCap = "4GB"
    }
...
appender('LOGZIO', LogzioLogbackAppender) {
    ...
    additionalFields="service=potic-token-renewer;env=${System.getenv('ENVIRONMENT_NAME') ?: 'dev'}"
}
